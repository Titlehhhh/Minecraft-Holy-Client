...
{
    public sealed class Protocol_756 : ProtocolBase
    {
        public Protocol_756(IPacketBroker client) : base(client)
        {
            SupportedVersion = 756;
        }
            ...
            switch (packet.Id)
            {
                ...
                case 0x0a:
                    if (_ontile_entity_data.HasObservers)
                    {
                        scoped var reader = new MinecraftPrimitiveReaderSlim(packet.Data);
                        Position location = reader.ReadPosition();
                        byte action = reader.ReadUnsignedByte();
    ...
    public class PacketLogin
    {
        public PacketLogin(int entityId, bool isHardcore, byte gameMode, sbyte previousGameMode, string[] worldNames,
            ...
            int viewDistance, bool reducedDebugInfo, bool enableRespawnScreen, bool isDebug, bool isFlat)
        {
            EntityId = entityId;
