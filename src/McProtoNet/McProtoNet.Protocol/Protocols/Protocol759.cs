...
namespace McProtoNet.Protocol759;

public sealed class Protocol_759 : ProtocolBase
{
    ...
    private readonly Subject<PacketChatPreview> _onchat_preview = new();
    ...
    private readonly Subject<PacketPlayerChat> _onplayer_chat = new();
    ...
    private readonly Subject<PacketShouldDisplayChatPreview> _onshould_display_chat_preview = new();
    ...
    public Protocol_759(IPacketBroker client) : base(client)
    {
    }
    ...
    public IObservable<PacketChatPreview> OnChatPreviewPacket => _onchat_preview;
    ...
    public IObservable<PacketPlayerChat> OnPlayerChatPacket => _onplayer_chat;
    ...
    public IObservable<PacketShouldDisplayChatPreview> OnShouldDisplayChatPreviewPacket =>
        _onshould_display_chat_preview;
    ...
    public Task SendChatMessage(string message, long timestamp, long salt, byte[] signature, bool signedPreview)
    {
        scoped var writer = new MinecraftPrimitiveWriterSlim();
    ...
    public Task SendChatPreview(int query, string message)
    {
        scoped var writer = new MinecraftPrimitiveWriterSlim();
    ...
    public Task SendBlockDig(int status, Position location, sbyte face, int sequence)
    {
        scoped var writer = new MinecraftPrimitiveWriterSlim();
    ...
    protected override void OnPacketReceived(InputPacket packet)
    {
        switch (packet.Id)
        {
            ...
            case 0x0b:
                ...
                break;
            case 0x0c:
                if (_onchat_preview.HasObservers)
                {
                    scoped var reader = new MinecraftPrimitiveReaderSlim(packet.Data);
                    var queryId = reader.ReadSignedInt();
                    string? message = null;
                    if (reader.ReadBoolean()) message = reader.ReadString();
                    _onchat_preview.OnNext(new PacketChatPreview(queryId, message));
                }
            ...
            case 0x2f:
                ...
                break;
            case 0x30:
                if (_onplayer_chat.HasObservers)
                {
                    scoped var reader = new MinecraftPrimitiveReaderSlim(packet.Data);
                    var signedChatContent = reader.ReadString();
                    string? unsignedChatContent = null;
                    if (reader.ReadBoolean()) unsignedChatContent = reader.ReadString();
                    var type = reader.ReadVarInt();
                    var senderUuid = reader.ReadUUID();
                    var senderName = reader.ReadString();
                    string? senderTeam = null;
                    if (reader.ReadBoolean()) senderTeam = reader.ReadString();
                    var timestamp = reader.ReadSignedLong();
                    var salt = reader.ReadSignedLong();
                    var tempArrayLength_8_0 = reader.ReadVarInt();
                    var signature = reader.ReadBuffer(tempArrayLength_8_0);
                    _onplayer_chat.OnNext(new PacketPlayerChat(signedChatContent, unsignedChatContent, type, senderUuid,
                        senderName, senderTeam, timestamp, salt, signature));
                }
            ...
            case 0x49:
                ...
                break;
            case 0x4b:
                if (_onshould_display_chat_preview.HasObservers)
                {
                    scoped var reader = new MinecraftPrimitiveReaderSlim(packet.Data);
                    var shouldDisplayChatPreview = reader.ReadBoolean();
                    _onshould_display_chat_preview.OnNext(new PacketShouldDisplayChatPreview(shouldDisplayChatPreview));
                }
            ...
            case 0x5d:
                if (_onsound_effect.HasObservers)
                {
                    ...
                    var pitch = reader.ReadFloat();
                    var seed = reader.ReadSignedLong();
                    _onsound_effect.OnNext(new PacketSoundEffect(soundId, soundCategory, x, y, z, volume, pitch, seed));
                }
            ...
            case 0x5f:
                if (_onsystem_chat.HasObservers)
                {
                    ...
                    var type = reader.ReadVarInt();
                    _onsystem_chat.OnNext(new PacketSystemChat(content, type));
                }
            ...
            case 0x3f:
                if (_onserver_data.HasObservers)
                {
                    scoped var reader = new MinecraftPrimitiveReaderSlim(packet.Data);
                    string? motd = null;
                    if (reader.ReadBoolean()) motd = reader.ReadString();
                    string? icon = null;
                    if (reader.ReadBoolean()) icon = reader.ReadString();
                    var previewsChat = reader.ReadBoolean();
                    _onserver_data.OnNext(new PacketServerData(motd, icon, previewsChat));
                }
...
public class PacketChatPreview
{
...
{
    public PacketPlayerChat(string signedChatContent, string? unsignedChatContent, int type, Guid senderUuid,
        string senderName, string? senderTeam, long timestamp, long salt, byte[] signature)
    {
    ...
    public string SignedChatContent { get; internal set; }
    public string? UnsignedChatContent { get; internal set; }
    public int Type { get; internal set; }
    public Guid SenderUuid { get; internal set; }
    public string SenderName { get; internal set; }
    public string? SenderTeam { get; internal set; }
    public long Timestamp { get; internal set; }
    public long Salt { get; internal set; }
...
public class PacketShouldDisplayChatPreview
{
    public PacketShouldDisplayChatPreview(bool should_display_chat_preview)
    {
        ShouldDisplayChatPreview = should_display_chat_preview;
    }

    public bool ShouldDisplayChatPreview { get; internal set; }
}
...
{
    public PacketEntitySoundEffect(int soundId, int soundCategory, int entityId, float volume, float pitch, long seed)
    {
        SoundId = soundId;
        SoundCategory = soundCategory;
        EntityId = entityId;
...
{
    public PacketSoundEffect(int soundId, int soundCategory, int x, int y, int z, float volume, float pitch, long seed)
    {
        SoundId = soundId;
        SoundCategory = soundCategory;
        X = x;
...
{
    public PacketSystemChat(string content, int type)
    {
        Content = content;
        Type = type;
    }
...
{
    public PacketServerData(string? motd, string? icon, bool previewsChat)
    {
        Motd = motd;
        Icon = icon;
    ...
    public bool PreviewsChat { get; internal set; }
}
