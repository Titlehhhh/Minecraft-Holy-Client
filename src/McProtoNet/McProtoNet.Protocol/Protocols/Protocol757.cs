...
namespace McProtoNet.Protocol757
{
    ...
    {
        public Protocol_757(IPacketBroker client) : base(client)
        {
            SupportedVersion = 757;
        }
            ...
            switch (packet.Id)
            {
                ...
                case 0x26:
                    if (_onlogin.HasObservers)
                    {
                        scoped var reader = new MinecraftPrimitiveReaderSlim(packet.Data);
                        int entityId = reader.ReadSignedInt();
                        bool isHardcore = reader.ReadBoolean();
                        byte gameMode = reader.ReadUnsignedByte();
                        sbyte previousGameMode = reader.ReadSignedByte();
                        var tempArrayLength_4_0 = reader.ReadVarInt();
                        var tempArray_4_0 = new string[tempArrayLength_4_0];
                        for (int i_4_0 = 0; i_4_0 < tempArrayLength_4_0; i_4_0++)
                        {
                            var for_item_4_0 = reader.ReadString();
                            tempArray_4_0[i_4_0] = for_item_4_0;
                        }

                        string[] worldNames = tempArray_4_0;
                        NbtTag dimensionCodec = reader.ReadNbt();
                        NbtTag dimension = reader.ReadNbt();
                        string worldName = reader.ReadString();
                        long hashedSeed = reader.ReadSignedLong();
                        int maxPlayers = reader.ReadVarInt();
                        int viewDistance = reader.ReadVarInt();
                        int simulationDistance = reader.ReadVarInt();
                        bool reducedDebugInfo = reader.ReadBoolean();
                        bool enableRespawnScreen = reader.ReadBoolean();
                        ...
                        _onlogin.OnNext(new PacketLogin(entityId, isHardcore, gameMode, previousGameMode, worldNames,
                            dimensionCodec, dimension, worldName, hashedSeed, maxPlayers, viewDistance,
                            simulationDistance, reducedDebugInfo, enableRespawnScreen, isDebug, isFlat));
                    }
                ...
                case 0x3d:
                    if (_onrespawn.HasObservers)
                    {
                        ...
                        bool isDebug = reader.ReadBoolean();
                        bool isFlat = reader.ReadBoolean();
                        bool copyMetadata = reader.ReadBoolean();
                        _onrespawn.OnNext(new PacketRespawn(dimension, worldName, hashedSeed, gamemode,
                            previousGamemode, isDebug, isFlat, copyMetadata));
                    }
    ...
    {
        public PacketLogin(int entityId, bool isHardcore, byte gameMode, sbyte previousGameMode, string[] worldNames,
            NbtTag dimensionCodec, NbtTag dimension, string worldName, long hashedSeed, int maxPlayers,
            int viewDistance, int simulationDistance, bool reducedDebugInfo, bool enableRespawnScreen, bool isDebug,
            bool isFlat)
        {
            EntityId = entityId;
        ...
        public sbyte PreviousGameMode { get; internal set; }
