...
public sealed class Protocol_760 : ProtocolBase
{
    ...
    private readonly Subject<PacketMessageHeader> _onmessage_header = new();
    ...
    public Protocol_760(IPacketBroker client) : base(client)
    {
    }
    ...
    public IObservable<PacketMessageHeader> OnMessageHeaderPacket => _onmessage_header;
    ...
    protected override void OnPacketReceived(InputPacket packet)
    {
        switch (packet.Id)
        {
            ...
            case 0x17:
                ...
                break;
            case 0x18:
                if (_onhide_message.HasObservers)
                {
                    scoped var reader = new MinecraftPrimitiveReaderSlim(packet.Data);
                    var tempArrayLength_0_0 = reader.ReadVarInt();
                    var signature = reader.ReadBuffer(tempArrayLength_0_0);
                    _onhide_message.OnNext(new PacketHideMessage(signature));
                }
            ...
            case 0x42:
                if (_onserver_data.HasObservers)
                {
                    ...
                    _onserver_data.OnNext(new PacketServerData(motd, icon, previewsChat, enforcesSecureChat));
                }
            ...
            case 0x5a:
                ...
                break;
            case 0x32:
                if (_onmessage_header.HasObservers)
                {
                    scoped var reader = new MinecraftPrimitiveReaderSlim(packet.Data);
                    ...
                    {
                        var tempArrayLength_1_0 = reader.ReadVarInt();
                        previousSignature = reader.ReadBuffer(tempArrayLength_1_0);
                    }

                    var senderUuid = reader.ReadUUID();
                    var tempArrayLength_2_0 = reader.ReadVarInt();
                    var signature = reader.ReadBuffer(tempArrayLength_2_0);
                    var tempArrayLength_3_0 = reader.ReadVarInt();
                    var messageHash = reader.ReadBuffer(tempArrayLength_3_0);
                    _onmessage_header.OnNext(new PacketMessageHeader(previousSignature, senderUuid, signature,
                        messageHash));
                }
...
public class PacketHideMessage
{
...
{
    public PacketMessageHeader(byte[]? previousSignature, Guid senderUuid, byte[] signature, byte[] messageHash)
    {
    ...
    public byte[]? PreviousSignature { get; internal set; }
    public Guid SenderUuid { get; internal set; }
    public byte[] Signature { get; internal set; }
    public byte[] MessageHash { get; internal set; }
}