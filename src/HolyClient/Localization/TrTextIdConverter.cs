...
public class TrTextIdConverter : IValueConverter
{
    ...
    {
        var textId = PreConverter?.Convert(value, null, null, null)?.ToString() ?? value?.ToString();
        return Prefix + (string.IsNullOrEmpty(textId)
                   ? ""
                   : Loc.Tr(string.Format(TextIdStringFormat, textId), DefaultText?.Replace("[apos]", "'"),
                       LanguageId)) +
               Suffix;
    }
