...
public class AssemblyViewModel : IAssemblyViewModel, IDisposable
{
    ...
    public AssemblyViewModel(IAssemblyFile assembly, IAssemblyManager assemblyManager)
    {
        ...
        DeleteCommand = ReactiveCommand.CreateFromTask(async () =>
        {
            ...
            {
                var stressTest = Locator.Current.GetService<IStressTestProfile>();

                if (stressTest.CurrentState != StressTestServiceState.None) await stressTest.Stop();
            ...
            else if (answer == ConfirmDeleteAssemblyAnswer.WaitAndDelete)
            {
                throw new NotImplementedException(
                    "Удаление сборки после остановки всех плагинов пока не поддерживается");
            }
        }).DisposeWith(d);
