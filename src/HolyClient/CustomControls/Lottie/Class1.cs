...
public class Lottie : Control
{
    ...
    {
        if (_animation == null) return new Size();

        var sourceSize = _animation is not null
    ...
    private Animation? Load(string path, Uri? baseUri)
    {
        ...
        if (assetStream is null) return default;
...
internal class LottieCompositionCustomVisualHandler : CompositionCustomVisualHandler
{
    ...
    {
        if (message is not LottiePayload msg) return;
    ...
    {
        if (_animation is null) return 0f;
    ...
    private void Draw(SKCanvas canvas)
    {
        ...
        var t = GetFrameTime();
        if (!_running) t = (float)animation.Duration.TotalSeconds;
