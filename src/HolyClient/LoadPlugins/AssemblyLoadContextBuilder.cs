...
public class AssemblyLoadContextBuilder
{
    ...
    ///     Use this feature if the <see cref="AssemblyLoadContext" /> of the <see cref="Assembly" /> is not the Runtime's
    ...
    {
        _defaultLoadContext = context ?? throw new ArgumentException(
            $"Bad Argument: AssemblyLoadContext in {nameof(AssemblyLoadContextBuilder)}.{nameof(SetDefaultContext)} is null.");
        return this;
    }
    ...
    public AssemblyLoadContextBuilder PreferDefaultLoadContextAssembly(AssemblyName assemblyName)
    {
        ...
        while (names.TryDequeue(out var name))
        {
            ...
            foreach (var reference in assembly.GetReferencedAssemblies()) names.Enqueue(reference);
        }
    ...
    {
        if (string.IsNullOrEmpty(path)) throw new ArgumentException("Value must not be null or empty.", nameof(path));
