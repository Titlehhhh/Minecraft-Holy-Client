...
namespace QuickProxyNet
{
	internal static class SocksHelper
	{
		...
		internal static async ValueTask EstablishSocks5TunnelAsync(Stream stream, string host, int port, NetworkCredential? credentials, bool async)
		{
		...
		internal static async ValueTask EstablishSocks4TunnelAsync(Stream stream, bool isVersion4a, string host, int port, NetworkCredential? credentials, bool async)
		{
			byte[] buffer = ArrayPool<byte>.Shared.Rent(BufferSize);
			try
			{
				...
				else if (!isVersion4a)
				{
					...
					{
						throw new ProxyProtocolException("SR.net_socks_no_ipv4_address");
					}
		...
		private static void VerifyProtocolVersion(byte expected, byte version)
		{
			if (expected != version)
			{
				throw new ProxyProtocolException($"SR.Format(SR.net_socks_unexpected_version: expected: {expected} version: {version}");
			}
		}
